...
import styles from './animated-splash-button.module.css';

interface AnimatedSplashButtonProps {
  onComplete: () => void;
  label: string;
  disabled?: boolean;
...
function AnimatedLabel({ className, label }: { className: string; label: string }) {
  return (
    <span className={`${styles.char} ${className}`}>
      {label.split('').map((character, index) => (
        <span key={index} className={character === ' ' ? styles.space : undefined} data-label={character === ' ' ? '\u00a0' : character} style={{ '--i': index + 1 } as React.CSSProperties}>{character === ' ' ? '\u00a0' : character}</span>
      ))}
    </span>
  );
}

export function AnimatedSplashButton({ onComplete, label, disabled = false, colorScheme = 'blue' }: AnimatedSplashButtonProps) {
  ...
  return (
    <button type="button" className={styles.button} onClick={handleClick} disabled={disabled} aria-label={label} style={{
      fontSize: label.length > 24 ? 12 : label.length > 16 ? 15 : 23,
      ...(colorScheme === 'green' ? { '--purple-100': '#d7f5e6', '--purple-200': '#b2ebce', '--purple-300': '#8de1b5', '--purple-400': '#3fc079', '--purple-500': '#2e915d' } : {}),
      ...
      <div className={styles.wrap}>
        ...
        <div className={styles.content}>
          <AnimatedLabel className={styles.state1} label={label} />
          <div className={styles.icon}><div /></div>
          <AnimatedLabel className={styles.state2} label={label} />
        </div>
      </div>
    </button>
  );
